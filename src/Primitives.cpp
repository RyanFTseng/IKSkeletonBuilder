...
void Joint::draw() {
   ...
   //
	...
	if (parent != NULL) {
		...
		glm::vec3 v =glm::vec3(parent->getPosition() - getPosition());
		glm::vec3 vn = glm::normalize(v);
		glm::mat4 rot = rotateToVector(glm::vec3(0, 1, 0), vn);
		glm::mat4 trans = glm::translate(getPosition());
		glm::mat4 shift = glm::translate(glm::vec3(0, glm::length(v) / 2, 0));

		ofPushMatrix();
			ofMultMatrix(trans*rot*shift);
			bone.height = glm::distance(parent->getPosition(), getPosition()) - 0.6;
			bone.draw();
		ofPopMatrix();
