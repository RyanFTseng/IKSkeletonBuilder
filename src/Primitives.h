...
public:
	Cone(int i) {
		position = glm::vec3(0, 0, 0);   // translate
		rotation = glm::vec3(0, 0, 0);   // rotate
		scale = glm::vec3(1, 1, 1);      // scale
		isSelectable = false;
	}
...
public:
	Joint(glm::vec3 p, float r, ofColor diffuse, string n) { position = p; radius = r; diffuseColor = diffuse; name = n; }
	void draw();
	Cone bone;
